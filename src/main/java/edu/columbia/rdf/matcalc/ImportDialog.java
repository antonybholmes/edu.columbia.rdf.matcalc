...
public class ImportDialog extends ModernDialogHelpWindow implements ModernClickListener {
   ...
   * The member spinner.
   */
  private final ModernCompactSpinner mIndexColsSpinner;

  /**
   * The member check header.
   */
  private final CheckBox mCheckHeader = new ModernCheckSwitch("Header rows",
      SettingsService.getInstance().getBool("matcalc.import.file.has-header"));

  /** The m check skip. */
  private final CheckBox mCheckSkip = new ModernCheckSwitch("Skip lines starting with", true); // SettingsService.getInstance().getBool("matcalc.import.file.skip.lines"));

  /** The m field skip. */
  private final ModernTextField mFieldSkip = new ModernClipboardTextField(
      SettingsService.getInstance().getString("matcalc.import.file.skip.matches"));

  private final CheckBox mNumericalCheck = new ModernCheckSwitch("Numerical", true);

  private final CheckBox mIndexColsCheck = new ModernCheckSwitch("Index columns", true);

  /** The m delimiter combo. */
  private final DelimiterCombo mDelimiterCombo = new DelimiterCombo();

  private final ModernCompactSpinner mHeaderSpinner;
   ...
   * @param isExcel the is excel
   */
  private void createUi(boolean isExcel) {
