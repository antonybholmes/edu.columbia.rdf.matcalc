...
public class ColumnGroupTreePanel extends ModernComponent {
   ...
   * The member tree.
   */
  protected ModernTree<XYSeries> mTree = new ModernTree<>();

  /**
   * The member load button.
   */
  private final ModernButton mLoadButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon(FolderBwVectorIcon.class, 16));

  /**
   * The member save button.
   */
  private final ModernButton mSaveButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon(SaveBwVectorIcon.class, 16));

  /**
   * The member add button.
   */
  private final ModernButton mAddButton = new ModernDialogThemeButton(
      AssetService.getInstance().loadIcon(PlusVectorIcon.class, 16));

  /** The m import button. */
  private final ModernButton mImportButton = new ModernDialogThemeButton("Import");

  /**
   * The member remove button.
   */
  private final ModernButton mRemoveButton = new ModernDialogThemeButton(AssetService.getInstance().loadIcon("trash_bw", 16));
  ...
  private class LoadEvents implements ModernClickListener {
    ...
    public void clicked(ModernClickEvent e) {
      try {
        loadGroups();
      } catch (IOException | SAXException | ParserConfigurationException | ParseException e1) {
        e1.printStackTrace();
      }
    }
  }
  ...
  protected synchronized void loadXml(Path file) throws SAXException, IOException, ParserConfigurationException {
    List<XYSeries> groups = new ArrayList<>();
  ...
  protected void createTree(XYSeries group, boolean clear) {
    ...
    TreeNode<XYSeries> groupNode = new TreeNode<>(group.getName(), group);

    for (int i : XYSeries.findColumnIndices(mMatrix, group)) {
      TreeNode<XYSeries> childNode = new TreeNode<>(mMatrix.getColumnName(i));
