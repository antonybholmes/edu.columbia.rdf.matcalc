...
public class XYSeriesImportDialog extends ModernDialogTaskWindow implements ModernClickListener {
  ...
  private final CheckBox mCheckSelectAll = new ModernCheckBox("Select All", true);

  /**
   * The member load button.
   */
  private final ModernButton mFilterButton = new RibbonButton("Filter...",
      AssetService.getInstance().loadIcon(OpenFolderVectorIcon.class, 16));

  /** The m clear filter button. */
  private final ModernButton mClearFilterButton = new RibbonButton("Clear Filter",
      AssetService.getInstance().loadIcon(CrossVectorIcon.class, 16));

  /** The m groups matrix. */
  private final DataFrame mGroupsMatrix;
  ...
  private final CheckBox mCheckComplementaryGroup = new ModernCheckBox("Complementary groups", true);
  ...
  /** The m table. */
  private final XYSeriesTable mTable;
  ...
  private void setup() {
    ...
    for (int i = 0; i < mGroupsMatrix.getRows(); ++i) {
      ...
      if (!mSampleMap.containsKey(mGroupsMatrix.getText(i, 0))) {
        mSampleMap.put(name, new TreeSet<>());
      }
    ...
    mCheckSelectAll.addClickListener((ModernClickEvent e) -> {
      for (ModernCheckBox c : mUseMap.values()) {
        c.setSelected(mCheckSelectAll.isSelected());
      }
    });

    mFilterButton.addClickListener((ModernClickEvent e) -> {
      try {
        filter();
    ...
    mClearFilterButton.addClickListener((ModernClickEvent e) -> {
      clearFilter();
    });
  ...
  private void refresh() {
    ...
    List<XYSeries> filteredSeries = new ArrayList<>();
