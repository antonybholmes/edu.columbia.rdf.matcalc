...
public class HierarchicalClusteringDialog extends ModernDialogHelpWindow implements ModernClickListener {
   ...
   */
  private final ClusterDistanceMetricCombo mDistanceCombo = new ClusterDistanceMetricCombo();

  /**
   * The member linkage combo.
   */
  private final ModernComboBox mLinkageCombo = new ModernComboBox();

  /**
   * The cluster columns check.
   */
  private final CheckBox mCheckClusterColumns = new ModernCheckSwitch("Cluster columns", true);

  /**
   * The cluster rows check.
   */
  private final CheckBox mCheckClusterRows = new ModernCheckSwitch("Cluster rows");

  private final CheckBox mCheckShowHeatMap = new ModernCheckSwitch("Show heat map", true);

  /** The m check optimal leaf order. */
  private final CheckBox mCheckOptimalLeafOrder = new ModernCheckSwitch("Optimal leaf order", true);

  /**
   * The member check plot.
   */
  private final CheckBox mCheckPlot = new ModernCheckSwitch(PlotConstants.MENU_PLOT, true);
